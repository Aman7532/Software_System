#define ADMINNAME "Bankadministrator"
#define PASSWORD "iiitbss"
int addEmployee(int connectionFD);
int addManager(int connectionFD);
void modifyCE(int connectionFD, int modifyChoice);
void manageRole(int connectionFD);
int changeAdminPassword(int connectionFD);
char readBuffer[4096], writeBuffer[4096];

void admin(int connectionFD)
{
    int flag = 0;
    char password[20];
label1:
    ...
    while(1)
    {
        ...
        switch (choice) {
            ...
            case 4:
                 if (changeAdminPassword(connectionFD)) {
                    bzero(writeBuffer, sizeof(writeBuffer));
